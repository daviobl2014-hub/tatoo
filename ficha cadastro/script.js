...
document.getElementById("anamneseForm").addEventListener("submit", (e) => {
  ...
  dados.user_agent = navigator.userAgent;

  // Aqui você integra com seu backend / WhatsApp / e-mail
  console.log("DADOS DA FICHA:", dados);

  // Salva localmente como backup
  try {
    const backup = { ...dados };
    delete backup.assinatura; // muito grande
    sessionStorage.setItem("ficha_backup", JSON.stringify(backup));
  } catch (err) {}
  ...
  document.querySelector(".sheet").innerHTML = `
    <div class="success-card">
      ...
      <p>Suas informações foram salvas com sucesso.<br>
      Apresente este comprovante ao profissional antes da sessão.</p>
